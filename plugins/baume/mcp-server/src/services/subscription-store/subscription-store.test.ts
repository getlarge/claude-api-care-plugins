  ...
  describe('MemorySubscriptionStore', () => {
    ...
    describe('subscribe/unsubscribe', () => {
      test('should subscribe session to URI', async () => {
        await store.subscribe('session-1', 'aip://findings?id=review-123');

        const subscribers = await store.getSubscribers(
          'aip://findings?id=review-123'
        );
      ...
      test('should allow multiple sessions to subscribe to same URI', async () => {
        await store.subscribe('session-1', 'aip://findings?id=review-123');
        await store.subscribe('session-2', 'aip://findings?id=review-123');
        await store.subscribe('session-3', 'aip://findings?id=review-123');
      ...
      test('should allow session to subscribe to multiple URIs', async () => {
        await store.subscribe('session-1', 'aip://findings?id=review-1');
        await store.subscribe('session-1', 'aip://findings?id=review-2');
        await store.subscribe('session-1', 'aip://specs?id=spec-1');

        const subs1 = await store.getSubscribers('aip://findings?id=review-1');
        const subs2 = await store.getSubscribers('aip://findings?id=review-2');
        const subs3 = await store.getSubscribers('aip://specs?id=spec-1');
      ...
      test('should unsubscribe session from URI', async () => {
        ...
        await store.unsubscribe('session-1', 'aip://findings?id=review-123');
      ...
      test('should handle unsubscribe from non-existent subscription', async () => {
        // Should not throw
        await store.unsubscribe('session-1', 'aip://findings?id=nonexistent');

        const subscribers = await store.getSubscribers(
          'aip://findings?id=nonexistent'
        );
    ...
    describe('unsubscribeAll', () => {
      test('should remove session from all subscriptions', async () => {
        ...
        await store.subscribe('session-1', 'aip://findings?id=review-2');
        await store.subscribe('session-1', 'aip://specs?id=spec-1');
        await store.subscribe('session-2', 'aip://findings?id=review-1');
