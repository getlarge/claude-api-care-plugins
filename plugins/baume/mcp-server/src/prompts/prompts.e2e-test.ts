...
describe('MCP Prompts E2E', () => {
  ...
  describe('prompts/list', () => {
    it('should list available prompts', async () => {
      ...
      const codeLocatorPrompt = response.result.prompts.find(
        (p: { name: string }) => p.name === 'baume-code-locator'
      );
      assert.ok(codeLocatorPrompt, 'Should include baume-code-locator prompt');
      ...
      const baumeLookupPrompt = response.result.prompts.find(
        (p: { name: string }) => p.name === 'baume-lookup'
      );
      assert.ok(baumeLookupPrompt, 'Should include baume-lookup prompt');
      // Note: @platformatic/mcp does not pass 'title' to MCP protocol (only 'name' and 'description')
      assert.ok(baumeLookupPrompt.description, 'Should have description');
      assert.ok(
        baumeLookupPrompt.arguments,
        'Should have arguments definition'
      );
      assert.ok(
        Array.isArray(baumeLookupPrompt.arguments),
        'Arguments should be an array'
      );
    });
  });
  ...
  describe('baume-lookup prompt', () => {
    it('should generate baume lookup prompt with string AIP', async () => {
      const response = await client.send('prompts/get', {
        name: 'baume-lookup',
        arguments: {
          aip: '158',
