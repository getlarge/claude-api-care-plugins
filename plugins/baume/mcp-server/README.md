...
MCP server exposing Baume OpenAPI reviewer as tools for AI agents (Claude Code, Claude Desktop, etc.).
...
| Tool                | Description                            |
| ------------------- | -------------------------------------- |
| `baume-review`      | Analyze OpenAPI spec against AIP rules |
| `baume-apply-fixes` | Apply suggested fixes to a spec        |
| `baume-list-rules`  | List available AIP rules               |
...
- `findings`: Array of findings from `baume-review` (only those with `fix` property are applied)
...
- `reviewId` (string): Review ID from a previous `baume-review` call
...
| -------------------------------- | ---------------------- | ----------------------------------------------------------------------------- | ------------------------------------------ |
| `baume://findings?id={reviewId}` | Baume Review Findings  | Cached review findings by reviewId (may include code locations if correlated) | `application/json`                         |
| `baume://specs?id={specId}`      | Modified OpenAPI Specs | Modified OpenAPI specs after applying fixes                                   | `application/x-yaml` or `application/json` |

### Accessing Resources

Resources are automatically created when using tools:

1. **After `baume-review`**: Review findings are stored and accessible via `baume://findings?id={reviewId}`
2. **After `baume-apply-fixes`**: Modified specs are stored and accessible via `baume://specs?id={specId}`

**Example workflow:**

```typescript
// 1. Run review
const reviewResult = await client.callTool('baume-review', {
  specPath: '/path/to/openapi.yaml',
});
// reviewResult.reviewId = "abc123"

// 2. Access findings resource
const findings = await client.readResource('baume://findings?id=abc123');
// Returns full findings JSON

// 3. Apply fixes
const fixResult = await client.callTool('baume-apply-fixes', {
  reviewId: 'abc123',
  specPath: '/path/to/openapi.yaml',
});
...
const modifiedSpec = await client.readResource('baume://specs?id=xyz789');
