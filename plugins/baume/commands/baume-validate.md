...
```
/baume-validate {plan-path}
/baume-validate  (uses most recent plan)
```
...
2. **Re-run the review**:
   ...
   **Option A: MCP Tool (Recommended)**
   ...
   If the `mcp__baume__baume-review` tool is available:
   ...
   **Option B: Node.js CLI (Fallback)**
   ...
   Run same checks as `/baume-review`:
...
The API now passes Baume review. Ready to create PR?
```

## Example

User: `/baume-validate .baume/plans/2025-01-15-orders-api-plan.md`
