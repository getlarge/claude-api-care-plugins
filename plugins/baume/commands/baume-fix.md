---
description: Apply API rule fixes to OpenAPI specs. Supports automated fixing via CLI or manual phase-by-phase implementation from a plan.
...
If the `mcp__baume__baume-apply-fixes` tool is available, use it:

```
Use mcp__baume__baume-apply-fixes with:
...
npx baume-review openapi.yaml --fix --dry-run

# Apply fixes to a new file
npx baume-review openapi.yaml --fix --output openapi-fixed.yaml

# Apply fixes in-place (overwrite original)
npx baume-review openapi.yaml --fix --output openapi.yaml
```
...
import { OpenAPIFixer } from '@getlarge/baume-reviewer/fixer';
...
```
/baume-fix {plan-path}
/baume-fix {plan-path} --phase 1
/baume-fix {plan-path} --task 1.2
/baume-fix  (uses most recent plan)
```
...
Updated plan: .baume/plans/2025-01-15-orders-api-plan.md
...
Run /baume-validate to verify fixes, or continue with Phase 2?
```
...
$ npx baume-review api.yaml --fix --dry-run
...
$ npx baume-review api.yaml --fix -o api.yaml
...
User: `/baume-fix .baume/plans/2025-01-15-orders-api-plan.md`

Claude: "Phase 1 (Quick Wins) has 3 tasks pending.

**Automated fixes available:** Tasks 1.1 and 1.2 can be applied automatically.
Run `npx baume-review openapi.yaml --fix` first, then I'll handle Task 1.3 which needs code changes.

Or start with Task 1.1: Fix resource naming?"

User: `/baume-fix --phase 2`
...
1. Run automated fixer first: `npx baume-review api.yaml --fix -o api.yaml`
2. Re-run review to see remaining issues: `npx baume-review api.yaml`
3. Use `/baume-fix` for manual code changes that remain
