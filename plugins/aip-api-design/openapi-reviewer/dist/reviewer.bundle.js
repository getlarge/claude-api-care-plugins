...
// src/rules/helpers/path-utils.js
...
// src/rules/helpers/singleton.js
...
// src/rules/helpers/spec-utils.js
var HTTP_METHODS = [
  "get",
  "post",
  "put",
...
// src/rules/aip122/plural-resources.js
...
// src/rules/aip122/consistent-casing.js
...
// src/rules/aip122/nested-ownership.js
...
// src/rules/aip131/get-no-body.js
...
// src/rules/aip131/index.js
var rules3 = [getNoBody];

// src/rules/aip132/has-filtering.js
...
// src/rules/aip132/has-ordering.js
...
// src/rules/aip132/index.js
var rules4 = [hasFiltering, hasOrdering];

// src/rules/aip133/post-returns-created.js
...
// src/rules/aip133/index.js
var rules5 = [postReturnsCreated];

// src/rules/aip134/patch-over-put.js
...
// src/rules/aip134/index.js
var rules6 = [patchOverPut];

// src/rules/aip135/delete-idempotent.js
...
// src/rules/aip135/index.js
var rules7 = [deleteIdempotent];

// src/rules/aip155/idempotency-key.js
...
// src/rules/aip155/index.js
var rules8 = [idempotencyKey];

// src/rules/aip158/list-paginated.js
...
// src/rules/aip158/max-page-size.js
...
// src/rules/aip158/response-next-token.js
...
// src/rules/aip158/index.js
var rules9 = [listPaginated, maxPageSize, responseNextToken];

// src/rules/aip193/schema-defined.js
...
// src/rules/aip193/responses-documented.js
...
// src/rules/aip193/standard-codes.js
...
var OpenAPIReviewer = class {
  ...
  #buildRuleSet(config) {
    ...
    if (config.categories && config.categories.length > 0) {
      rules11 = getRulesByCategory(config.categories);
    }
...
function reviewSpec(spec, specPath) {
  const reviewer = new OpenAPIReviewer();
  return reviewer.review(spec, specPath);
}
function reviewSpecStrict(spec, specPath) {
...
function formatMarkdown(result) {
  ...
  lines.push("");
  const errors = result.findings.filter((f3) => f3.severity === "error");
  const warnings = result.findings.filter((f3) => f3.severity === "warning");
  const suggestions = result.findings.filter(
    (f3) => f3.severity === "suggestion"
  );
  if (errors.length > 0) {
    lines.push("### \u{1F534} Errors (MUST fix)");
    lines.push("");
  ...
  if (warnings.length > 0) {
    lines.push("### \u{1F7E1} Warnings (SHOULD fix)");
    lines.push("");
  ...
  if (suggestions.length > 0) {
    lines.push("### \u{1F4A1} Suggestions (MAY improve)");
    lines.push("");
...
function formatFixAsYAML(fix, indent = "") {
  ...
  if (fix.target !== void 0 && Object.keys(fix.target).length > 0) {
    lines.push(`${indent}target:`);
    for (const [key, value] of Object.entries(fix.target)) {
      lines.push(`${indent}  ${key}: ${formatYAMLValue(value)}`);
    }
  }
  if (fix.replacement !== void 0) {
    if (typeof fix.replacement === "object" && fix.replacement !== null && !Array.isArray(fix.replacement)) {
  ...
  if (fix.specChanges?.length) {
    ...
    for (const change of fix.specChanges) {
      ...
      if (change.from !== void 0)
        lines.push(`${indent}    from: "${change.from}"`);
      if (change.to !== void 0)
        lines.push(`${indent}    to: "${change.to}"`);
      if (change.value !== void 0) {
        if (typeof change.value === "object" && change.value !== null && !Array.isArray(change.value)) {
...
function formatConsole(result, useColors = true) {
  const c2 = useColors ? colors2 : noColors;
  const lines = [];
  lines.push("");
  lines.push(`${c2.bold}API Review: ${result.specPath}${c2.reset}`);
  lines.push("");
  const errors = result.findings.filter((f3) => f3.severity === "error");
  const warnings = result.findings.filter((f3) => f3.severity === "warning");
  const suggestions = result.findings.filter(
  ...
  if (errors.length > 0) {
    lines.push(`${c2.red}${c2.bold}Errors (${errors.length})${c2.reset}`);
    for (const finding of errors) {
      lines.push(formatFindingConsole(finding, c2));
    }
    lines.push("");
  }
  if (warnings.length > 0) {
    lines.push(`${c2.yellow}${c2.bold}Warnings (${warnings.length})${c2.reset}`);
    for (const finding of warnings) {
      lines.push(formatFindingConsole(finding, c2));
    }
    lines.push("");
  ...
  if (suggestions.length > 0) {
    lines.push(
      `${c2.blue}${c2.bold}Suggestions (${suggestions.length})${c2.reset}`
    );
    for (const finding of suggestions) {
      lines.push(formatFindingConsole(finding, c2));
    }
  ...
  if (result.findings.length === 0) {
    lines.push(`${c2.green}${c2.bold}\u2713 No issues found!${c2.reset}`);
    lines.push("");
  }
  const summaryParts = [];
  if (result.summary.errors > 0) {
    summaryParts.push(`${c2.red}${result.summary.errors} errors${c2.reset}`);
  }
  if (result.summary.warnings > 0) {
    summaryParts.push(
      `${c2.yellow}${result.summary.warnings} warnings${c2.reset}`
    );
  }
  if (result.summary.suggestions > 0) {
    summaryParts.push(
      `${c2.blue}${result.summary.suggestions} suggestions${c2.reset}`
    );
  }
...
function formatFindingConsole(finding, c2) {
  const lines = [];
  lines.push(`  ${c2.dim}${finding.ruleId}${c2.reset}`);
  lines.push(`    ${c2.cyan}${finding.path}${c2.reset}`);
  lines.push(`    ${finding.message}`);
  if (finding.suggestion) {
    lines.push(`    ${c2.dim}\u2192 ${finding.suggestion}${c2.reset}`);
  }
  return lines.join("\n");
}
...
function getAIPInfo(aip) {
  return AIP_METADATA[aip];
}
function getImplementedAIPs() {
  return Object.keys(AIP_METADATA).map(Number).sort((a2, b) => a2 - b);
}
...
function applyAllFixes(spec, findings, options = {}) {
...
//# sourceMappingURL=reviewer.bundle.js.map
